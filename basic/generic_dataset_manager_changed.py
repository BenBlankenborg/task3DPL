#Original code taken from https://github.com/CV-IP/VerticalAttentionOCR/tree/master/basic
...
class DatasetManager:
    ...
    def load_datasets(self):
        ...
        self.train_dataset = self.dataset_class(
            ...
            self.get_paths_and_sets("train", self.params["train"]["datasets"])
        )
        
        # Validation datasets
        for custom_name, valid_config in self.params["valid"].items():
            self.valid_datasets[custom_name] = self.dataset_class(
                self.params,
                "valid",
                custom_name,
                self.get_paths_and_sets("valid", valid_config["datasets"])
            )
        
        #for custom_name, valid_config in self.params["test"].items():
        #    self.test_datasets[custom_name] = self.dataset_class(
        #        self.params,
        #        "test",
        #        custom_name,
        #        self.get_paths_and_sets("test", test_config["datasets"])
         #   )
    ...
    def get_paths_and_sets(self, set_name, dataset_names):
        ...
        print(f"With dataset_names: {dataset_names} (type: {type(dataset_names)})")
        paths_and_sets = []
        print(f"Type of dataset_names: {type(dataset_names)}")  # Should be list
        print(f"Content: {dataset_names}")  # Should be ['GERMAN']
        if isinstance(dataset_names, str):
            print("Warning: dataset_names is a string, not a list!")
        # Ensure dataset_names is always treated as a list
        if isinstance(dataset_names, str):
            dataset_names = [dataset_names]  # Wrap single string in a list
        
        for dataset_name in dataset_names:
            path = self.params["datasets"].get(dataset_name)
            if path is None:
                raise ValueError(f"Dataset '{dataset_name}' not found. Available: {list(self.params['datasets'].keys())}")
            paths_and_sets.append({
                "path": path,
                "set_name": set_name
            })
        ...
        for i in range(len(self.samples)):
            ...
            for preprocessing in preprocessings:
                ...
                if preprocessing["type"] == "to_RGB":
                    ...
                    if c == 1:
                    # Grayscale -> RGB
                        self.samples[i]["img"] = np.concatenate([temp_img] * 3, axis=2)
                    
                    elif c == 4:
                        # RGBA -> RGB (drop alpha)
                        self.samples[i]["img"] = temp_img[:, :, :3]
                    
                    elif c != 3:
                        raise ValueError(f"Unsupported number of channels: {c}. Expected 1 (grayscale), 3 (RGB), or 4 (RGBA).")
